...
using namespace AscEmu::Scripts::InstanceScripts::VioletHold::Xevozz;
...
XevozzAI::XevozzAI(Creature* pCreature) : CreatureAIScript(pCreature)
{
    ...
    // Spells
    addAISpell(SPELL_ARCANE_BARRAGE_VOLLEY, 50.0f, TARGET_SOURCE, 0, 8);
    addAISpell(SPELL_ARCANE_BUFFET, 33.0f, 15, [this]() { return getBestUnitTarget(TargetFilter_InRangeOnly, 0.0f, 45.0f); });
...
void XevozzAI::AIUpdate(unsigned long time_passed)
{
    ...
    while (uint32_t eventId = scriptEvents.getFinishedEvent())
    {
        switch (eventId)
        {
            case 1:
            {
                sendRandomDBChatMessage({ SAY_REPEAT_SUMMON1, SAY_REPEAT_SUMMON2 }, nullptr);

                uint8_t spell = SelectRandomContainerElement(summonSpells);

                getCreature()->castSpell(getCreature(), EtherealSphereSummonSpells[spell]);
...
{
    if (info->getId() == SPELL_ARCANE_POWER || info->getId() == H_SPELL_ARCANE_POWER)
        sendDBChatMessage(SAY_SUMMON_ENERGY);
}
...
void EtherealSphereAI::OnLoad()
{
    ...
    getCreature()->castSpell(getCreature(), SPELL_POWER_BALL_VISUAL);
    getCreature()->castSpell(getCreature(), SPELL_POWER_BALL_DAMAGE_TRIGGER);
        ...
        switch (eventId)
        {
            case 1:
            {
                if (Creature* xevozz = mInstance->getCreatureFromData(DATA_XEVOZZ))
                {
                    ...
                    {
                        getCreature()->castSpell(nullptr, SPELL_ARCANE_POWER);
