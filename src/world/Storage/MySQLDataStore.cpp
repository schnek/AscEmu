...
void MySQLDataStore::loadAdditionalTableConfig()
{
    ...
    for (auto& str : strs)
    {
        ...
        sLogger.info("MySQLDataLoads : Table {} added as additional table for {}", additional_table, target_table);
    }
}
...
uint32_t const MySQLDataStore::getItemDisplayIdForEntry(uint32_t entry)
{
    if (entry != 0)
    {
        ...
        if (dbcDisplay != 0)
        {
            sLogger.debug("Item entry {} not available in item_properties or has an invalid displayId! Using dbcDisplayId {}!", entry, dbcDisplay);
            return dbcDisplay;
        }

        sLogger.debug("Invalid item entry {} is not in item_properties table or in Item.dbc! Please create a item_properties entry to return a valid displayId", entry);
#else
        return entry;
#endif
    }
...
void MySQLDataStore::loadCreaturePropertiesTable()
{
    ...
    do
    {
        ...
        {
            sLogger.failure("Table `creature_properties` MinHealth = 0 is not a valid value! Default set to 1 for entry: {}.", entry);
        ...
        {
            sLogger.failure("Table `creature_properties` MaxHealth = 0 is not a valid value! Default set to 1 for entry: {}.", entry);
        ...
        {
            sLogger.failure("Table `creature_properties` AttackType: {} is not a valid value! Default set to 0 for entry: {}.", fields[10].GetUInt32(), entry);
...
void MySQLDataStore::loadGameObjectQuestItemBindingTable()
{
    ...
    if (gameobject_quest_item_result != nullptr)
    {
        do
        {
            ...
            {
                sLogger.debug("Table `gameobject_quest_item_binding` includes data for invalid gameobject_properties entry: {}. Skipped!", entry);
                continue;
            }
            ...
            if (quest == nullptr)
            {
                sLogger.debug("Table `gameobject_quest_item_binding` includes data for invalid quest_properties : {}. Skipped!", quest_entry);
                continue;
            }
            else
            {
...
void MySQLDataStore::loadGameObjectQuestPickupBindingTable()
{
    ...
    if (gameobject_quest_pickup_result != nullptr)
    {
        do
        {
            ...
            if (gameobject_properties == nullptr)
            {
                sLogger.debug("Table `gameobject_quest_pickup_binding` includes data for invalid gameobject_properties entry: {}. Skipped!", entry);
                continue;
            }
            ...
            if (quest == nullptr)
            {
                sLogger.debug("Table `gameobject_quest_pickup_binding` includes data for invalid quest_properties : {}. Skipped!", quest_entry);
                continue;
            }
            else
            {
...
void MySQLDataStore::loadSpellClickSpellsTable()
{
    ...
    do
    {
        ...
        if (!cInfo)
        {
            sLogger.failure("Table npc_spellclick_spells references unknown creature_properties {}. Skipping entry.", npc_entry);
            continue;
        }
        ...
        if (!spellinfo)
        {
            sLogger.failure("Table npc_spellclick_spells creature: {} references unknown spellid {}. Skipping entry.", npc_entry, spellid);
            continue;
        }

        uint8_t userType = fields[3].GetUInt8();
        if (userType >= SPELL_CLICK_USER_MAX)
            sLogger.failure("Table npc_spellclick_spells creature: {} references unknown user type {}. Skipping entry.", npc_entry, uint32(userType));
    ...
    for (uint8_t _race = 0; _race < DBC_NUM_RACES; ++_race)
    {
        ...
        for (uint8_t _class = 0; _class < MAX_PLAYER_CLASSES; ++_class)
        {
            ...
            for (uint8_t level = 1; level < DBC_STAT_LEVEL_CAP; ++level)
            {
                ...
                {
                    sLogger.info("Race {} Class {} Level {} does not have stats data. Using stats data of level % i.", _race, _class, level + 1, level);
...
void MySQLDataStore::loadCreatureGroupSpawns()
{
    ...
    do
    {
        ...
        for (const auto creatureSpawnMap : sMySQLStore._creatureSpawnsStore)
        {
            for (const auto creatureSpawn : creatureSpawnMap)
            {
                if (creatureSpawn->id == spawnId)
                {
                    ...
                    {
                        sLogger.failure("Spawn group {} has map ID {}, but spawn ({}) has map id {} - spawn NOT added to group!", groupId, groupTemplate.mapId, spawnId, creatureSpawn->mapId);
                        continue;
                    }
        ...
        if (!data)
        {
            sLogger.failure("Spawn data with ID ({}) not found, but is listed as a member of spawn group {}!", spawnId, groupId);
            continue;
        } 
    } while (result->NextRow());
