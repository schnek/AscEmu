...
void Pet::DealDamage(Unit* pVictim, uint32 damage, uint32 /*targetEvent*/, uint32 /*unitEvent*/, uint32 spellId, bool no_remove_auras)
{
    ...
    if (pVictim->GetHealth() <= damage)
    {
        ...
        if (pVictim->IsPlayer())
        {
            ...
            if (setAurastateFlag)
            {
                addAuraState(AURASTATE_FLAG_LASTKILLWITHHONOR);

                if (!sEventMgr.HasEvent(m_Owner, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE))
                    sEventMgr.AddEvent(static_cast< Unit* >(m_Owner), &Unit::removeAuraState, AURASTATE_FLAG_LASTKILLWITHHONOR, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE, 20000, 1, 0);
                else
                    ...
                    else if (IsUnit())
                    {
                        ...
                        if (xp > 0)
                        {
                            ...
                            if (!sEventMgr.HasEvent(this, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE))
                                sEventMgr.AddEvent(static_cast<Unit*>(this), &Unit::removeAuraState, AURASTATE_FLAG_LASTKILLWITHHONOR, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE, 20000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
                            else
