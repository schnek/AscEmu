...
void Unit::GiveGroupXP(Unit* pVictim, Player* PlayerInGroup)
{
    ...
    else
    {
        ...
        for (int i = 0; i < active_player_count; i++)
        {
            ...
            active_player_list[i]->addAuraState(AURASTATE_FLAG_LASTKILLWITHHONOR);
            if (!sEventMgr.HasEvent(active_player_list[i], EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE))
            {
                sEventMgr.AddEvent(static_cast<Unit*>(active_player_list[i]), &Unit::removeAuraState, AURASTATE_FLAG_LASTKILLWITHHONOR, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE, 20000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
            }
            else
            {
...
void Unit::Strike(Unit* pVictim, uint32 weapon_damage_type, SpellInfo* ability, int32 add_damage, int32 pct_dmg_mod, uint32 exclusive_damage, bool disable_proc, bool skip_hit_check, bool force_crit)
{
    ...
    switch (r)
    {
        ...
        case 1:     //dodge
            ...
            pVictim->addAuraState(AURASTATE_FLAG_DODGE_BLOCK_PARRY);
            if (!sEventMgr.HasEvent(pVictim, EVENT_DODGE_BLOCK_FLAG_EXPIRE))
                sEventMgr.AddEvent(pVictim, &Unit::removeAuraState, AURASTATE_FLAG_DODGE_BLOCK_PARRY, EVENT_DODGE_BLOCK_FLAG_EXPIRE, 5000, 1, 0);
        ...
        case 2:     //parry
            ...
            if (pVictim->IsPlayer())
            {
                ...
                pVictim->addAuraState(AURASTATE_FLAG_PARRY);	                        //SB@L: Enables spells requiring parry
                if (!sEventMgr.HasEvent(pVictim, EVENT_PARRY_FLAG_EXPIRE))
                    sEventMgr.AddEvent(pVictim, &Unit::removeAuraState, AURASTATE_FLAG_PARRY, EVENT_PARRY_FLAG_EXPIRE, 5000, 1, 0);
                else
                ...
                {
                    pVictim->addAuraState(AURASTATE_FLAG_DODGE_BLOCK_PARRY);	                //SB@L: Enables spells requiring dodge
                    if (!sEventMgr.HasEvent(pVictim, EVENT_DODGE_BLOCK_FLAG_EXPIRE))
                        sEventMgr.AddEvent(pVictim, &Unit::removeAuraState, AURASTATE_FLAG_DODGE_BLOCK_PARRY, EVENT_DODGE_BLOCK_FLAG_EXPIRE, 5000, 1, 0);
                    else
...
void Unit::EventHealthChangeSinceLastUpdate()
{
    ...
    if (pct < 35)
        addAuraState(AURASTATE_FLAG_HEALTH35);
    else
        removeAuraState(AURASTATE_FLAG_HEALTH35);

    if (pct < 20)
        addAuraState(AURASTATE_FLAG_HEALTH20);
    else
        removeAuraState(AURASTATE_FLAG_HEALTH20);

#if VERSION_STRING >= WotLK
    if (pct > 75)
        addAuraState(AURASTATE_FLAG_HEALTH75);
    else
        removeAuraState(AURASTATE_FLAG_HEALTH75);
#endif
}
