...
{
    class MsgMoveSetRunBackSpeed : public ManagedPacket
    {
    public:
        WoWGuid guid;
        float rate;

        MsgMoveSetRunBackSpeed() : MsgMoveSetRunBackSpeed(WoWGuid(), 0)
        {
        }

        MsgMoveSetRunBackSpeed(WoWGuid guid, float rate) :
//Zyres: Due to wrong understanding of these opcodes the logic gets turned around here
            ...
            ManagedPacket(SMSG_MOVE_SET_RUN_BACK_SPEED, 0),
#endif
            guid(guid),
            rate(rate)
        {
