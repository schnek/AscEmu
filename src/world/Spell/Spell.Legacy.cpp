...
uint8 Spell::prepare(SpellCastTargets* targets)
{
    ...
    else
    {
        ...
        // TODO: This is partially correct but partially not
        // afaik only some aurastates should be removed when casting a specific spell
        // but I don't think removing the aurastate already in ::prepare is the correct way
        // Commenting this out for now but it needs more investigation -Appled

        // aura state removal
        /*if (GetSpellInfo() && GetSpellInfo()->getCasterAuraState() != AURASTATE_NONE && GetSpellInfo()->getCasterAuraState() != AURASTATE_FLAG_JUDGEMENT)
            if (u_caster != nullptr)
                u_caster->removeAuraState(AuraState(GetSpellInfo()->getCasterAuraState()));*/
    }
    ...
    if (cancastresult == SPELL_CANCAST_OK)
    {
        ...
        if (p_caster)
        {
            switch (GetSpellInfo()->getId())
            {
                ...
                case 34428:
                {
                    p_caster->removeAuraState(AURASTATE_FLAG_LASTKILLWITHHONOR);
                } break;
            ...
            if (!IsReflected())
            {
                ...
                if (p_caster && p_caster->IsInWorld())
                {
                    for (i = UniqueTargets.begin(); i != UniqueTargets.end(); ++i)
                    {
                        ...
                        //Target->removeAuraState(AuraState(GetSpellInfo()->getTargetAuraState()));
                    }
                }
            }
