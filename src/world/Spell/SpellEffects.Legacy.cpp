...
void Spell::SpellEffectProspecting(uint8_t /*effectIndex*/)
{
    ...
    {
        sLogger.debugFlag(AscEmu::Logging::LF_SPELL_EFF, "Successfully prospected item {}", uint32(itemTarget->getEntry()));
        p_caster->sendLoot(itemTarget->getGuid(), LOOT_PROSPECTING, p_caster->GetMapId());
    }
    else // this should never happen either
    {
        sLogger.debugFlag(AscEmu::Logging::LF_SPELL_EFF, "Prospecting failed, item {} has no loot", uint32(itemTarget->getEntry()));
        sendCastResult(SPELL_FAILED_CANT_BE_PROSPECTED);
    }
}
...
void Spell::SpellEffectForgetSpecialization(uint8_t effectIndex)
{
    ...
    sLogger.debugFlag(AscEmu::Logging::LF_SPELL_EFF, "Player {} have forgot spell {} from spell {} (caster: {})", playerTarget->getGuidLow(), spellid, getSpellInfo()->getId(), m_caster->getGuidLow());
}
...
void Spell::SpellEffectMilling(uint8_t /*effectIndex*/)
{
    ...
    if (itemTarget->loot->items.size() > 0)
    {
        sLogger.debugFlag(AscEmu::Logging::LF_SPELL_EFF, "Successfully milled item {}", uint32(itemTarget->getEntry()));
        p_caster->sendLoot(itemTarget->getGuid(), LOOT_MILLING, p_caster->GetMapId());
    }
    else // this should never happen either
    {
        sLogger.debugFlag(AscEmu::Logging::LF_SPELL_EFF, "Milling failed, item {} has no loot", uint32(itemTarget->getEntry()));
        sendCastResult(SPELL_FAILED_CANT_BE_PROSPECTED);
    }
}
