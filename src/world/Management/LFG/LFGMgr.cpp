...
void LfgMgr::Join(Player* player, uint8 roles, const LfgDungeonSet& selectedDungeons, const std::string& comment)
{
    ...
    if (joinData.result != LFG_JOIN_OK)
    {
        sLogger.debug("{} joining with {} members. result: {}   cant join return", guid, grp ? grp->MemberCount() : 1, joinData.result);
    ...
    sLogger.debug("{} joined with {} members. dungeons: {}", guid, grp ? grp->MemberCount() : 1, uint8(dungeons.size()));
}
