...
uint32 ReadBuild(int locale)
{
    ...
    HANDLE dbcFile;
    if (!SFileOpenFileEx(LocaleMpq, filename.c_str(), SFILE_OPEN_FROM_MPQ, &dbcFile)) //\sch: SFILE_OPEN_PATCHED_FILE
    {
...
uint32 ReadMapDBC()
{
    ...
    HANDLE dbcFile;
    if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\Map.dbc", SFILE_OPEN_FROM_MPQ, &dbcFile)) //\sch: SFILE_OPEN_PATCHED_FILE
    {
...
void ReadAreaTableDBC()
{
    ...
    HANDLE dbcFile;
    if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\AreaTable.dbc", SFILE_OPEN_FROM_MPQ, &dbcFile)) //\sch: SFILE_OPEN_PATCHED_FILE
    {
...
void ReadLiquidTypeTableDBC()
{
    ...
    HANDLE dbcFile;
    if (!SFileOpenFileEx(LocaleMpq, "DBFilesClient\\LiquidType.dbc", SFILE_OPEN_FROM_MPQ, &dbcFile)) //\sch: SFILE_OPEN_PATCHED_FILE
    {
...
void ExtractDBCFiles(int l, bool basicLocale)
{
    ...
    if (listFile)
    {
        ...
        do
        {
            if (!SFileOpenFileEx(LocaleMpq, foundFile.cFileName, SFILE_OPEN_FROM_MPQ, &dbcFile)) //\sch: SFILE_OPEN_PATCHED_FILE
            {
